...
if [[ -n "${TEST_PARAM_DRIFT_JOB:+x}" ||  "$#" -gt 0 ]]; then  ## Fails if TEST_PARAM_DRIFT_JOB is unset or empty
    ...
    ARGLIST=`getopt -o 's:r:d:m:t:q:' --long 'sync:,recipe:,disk:,mountpoint:,tag:,drifttype:' -n $0 -- $DRIFTCMD`
    if [ $? -ne 0 ]; then
	usage_msg;
	exit 1;
    fi
...
if [[ -e /root/storagePASS ]] 
	...
	echo "run_drift.py -R ${RECIPE} -D ${DISK} -M ${MOUNTPOINT} -F ${FSYSTEM}"

	python run_drift.py -R ${RECIPE} -D ${DISK} -M ${MOUNTPOINT} -F ${FSYSTEM}
