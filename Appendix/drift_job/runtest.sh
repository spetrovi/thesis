...
if [[ -n "${TEST_PARAM_DRIFT_JOB:+x}" ||  "$#" -gt 0 ]]; then  ## Fails if TEST_PARAM_DRIFT_JOB is unset or empty
    ...
    ARGLIST=`getopt -o 's:r:d:M:t:q:m:f:' --long 'sync:,recipe:,disk:,mountpoint:,tag:,drifttype:,maintain:,fstrim:' -n $0 -- $DRIFTCMD`
    if [ $? -ne 0 ]; then
	usage_msg;
	exit 1;
    fi
	...
	case $1 in
	    ...
	    -d|--disk)        shift;    DISK="$1";;
            ...
            -m|--maintain)    shift;    MAINTAIN="$1";;
            -f|--fstrim)      shift;    FSTRIM="$1";;
	    -h|--help)		usage_msg;
    				exit 1;;
	    --)			break;;
    	esac
...
if [[ -e /root/storagePASS ]] 
	...
	RESULT="./$(date '+%Y-%b-%d_%Hh%Mm%Ss')-drift_job-$FSYSTEM-$TAG-$DRIFTTYPE"
	RESULT=${RESULT//[[:space:]]/}
	...
	tar -Jxvf ./fs-drift.tar.xz
	echo "Running run_drift.py">>./out/log.out

	echo "run_drift.py -R ${RECIPE} -D ${DISK} -M ${MOUNTPOINT} -F ${FSYSTEM} -M ${MAINTAIN} -T ${TAG} -f ${FSTRIM}"

	python run_drift.py -R ${RECIPE} -D ${DISK} -m ${MOUNTPOINT} -F ${FSYSTEM} -M ${MAINTAIN} -T ${TAG} -f ${FSTRIM}
	...
	if [ "$FSYSTEM" == "ext4" ]
		then
			...
			xfs_metadump -o $DISK - |bzip2 > $RESULT.bz2
	fi
	echo "result is ${RESULT}" >> ./out/log.out
	...
	cd out
	echo "Sending output on server" >> ./out/log.out
	logs = ${logs:-`ls *.out`}
	for log in $logs;do
	...
	if [[ ${doRSYNC} == 1  && -e ./out/log.out ]]
	  then
	       ...
	       rsync ${RSYNC_OPTIONS} ${TMPD}/ ${RSYNC_SERVER}/fsage_thesis/${RESULTS_ROOT_DIRECTORY}/${HOST}_images
	       #rsync ${RSYNC_OPTIONS} ${TMPD}/ ${RSYNC_SERVER}/fsage_thesis/${RESULTS_ROOT_DIRECTORY}/${HOST}_images

	       #rsync ${RSYNC_OPTIONS} *.bz2 ${RSYNC_SERVER}/fsage_thesis/${RESULTS_ROOT_DIRECTORY}/${HOST}_images

	       RSYNC_DEST="${RSYNC_SERVER}/fsage_thesis/${RESULTS_ROOT_DIRECTORY}/${HOST}_images"
	       ...
	       rsync ${RSYNC_OPTIONS} *.bz2 ${RSYNC_DEST}
