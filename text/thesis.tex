...
\footnotetext[2]{The mechanism of space releasing from file sytem is described in next chapter}
...
\chapter{Results}
This chapter present results of implemented tests. In the first section, I describe means of data evaluation. In the second section, I show elementary differences between fresh and aged file systems. In the third section, I describe differences between two tested file systems (XFS, Ext4) in terms of aging. In fourth section I demonstrate the effect file system aging have on underlying storage.
...
To find out fragmentation of used space, \textit{xfs\_io fiemap} were used. This tool can display physical mapping of any file, using $\SI{512}{\byte}$ blocks. Therefore, if run on every file in file system, histogram of fragments of used space can be computed.
