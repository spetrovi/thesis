...
The file sysem aging tool succesfully induced fragmentation in all performed tests. From the data generated by aging tool, it is apparent, that the aging process may negatively affects performance of the file systems. Performance degradation varies with used storage, file system implementation and maintainance techniques (i.e. \texttt{fstrim}).

%durden W495 xfs vs draven
On Figure X, we can see evolution of fragmentation of free space gathered while testing at lower file system utilisation (up to 80\%). From the figure X, we can see that with higher file system utilisation, free space becomes tremendously fragmented.

Despite this occurence, it is remarkable, that fragmentation of used space is very similar accross these two test runs. On Figure X and Y, we can see, both file systems have about 92\% of files optimally allocated. Considering amount of fragments of free space differs in orders of tens of thousands, it is remarkable, that amount of non-contiguous extents of free files differs only by 33\%
...
XFS and EXT4 have very similar disk layouts in spite of free and used space fragmentation. After maximal disk utilisation both file systems have up to 50\,000 fragments. At the end of the test, both file systems had 98\% of files optimally allocated. The only difference when considering file block layout seems to be in size distribution of extents. As can be seen on Figure , EXT4 has more smaller fragments than XFS. Since continuity of relevant data may play role even while using SSD, this could mean potential performance penalty.
...
\subsection{XFS and EXT4 on NVMe}
Filesystemy
NVME xfs vs NVME ext4
%***cakame na NVME ext4***


300 HDD xfs vs 300 HDD ext4
%***cakame na vsetko***
...
600 HDD xfs vs 300 HDD xfs
***cakame na 300 HDD xfs***

\begin{figure}[ht]
    \centering
    \fbox{\includegraphics[width=\textwidth,keepaspectratio,trim=4 4 4 4,clip]{../charts/HDD_xfs/screenshot.png}}
    \caption{Evolution of free space fragmentation using XFS}
    \label{fig:hdd_xfs_free_frag}
\end{figure}

\begin{figure}[ht]
    \centering
    \fbox{\includegraphics[width=\textwidth,keepaspectratio,trim=4 4 4 4,clip]{../charts/HDD_ext4/screenshot.png}}
    \caption{Evolution of free space fragmentation using Ext4}
    \label{fig:hdd_ext4_free_frag}
\end{figure}
...
Running \texttt{fstrim} regularly through the test has significant impact on perofmance on operations create and append. Those operations create IO requests sequentially, so it is expected
