...
\thesislong{thanks}{
I would like to thank to my technical advisory Matúš Kocka for tremendous help and guidenance. I would also like to thank Red Hat for providing necessary testing environment. 
}
...
\footnotetext[1]{Savitzky-Golay filter is a tool for smooothing noisy data...}
...
From the test conducted on highly utilised  HDD, we can conclude that  XFS fights free space fragmentation much better. On a Figure~\ref{xfs99_free}, which shows last 6 hours of testing high utilisation of XFS on HDD, we can see relatively low amount of free space extents. The peaks in number of extents are caused by random space releasing conducted by aging test, which can be confirmed by examining correspoding untilisation evolution chart (Figure~\ref{xfs99_usage}). After every peak occurence, it is clear that number of small extents is progressively lower until the next peak. It is clear that XFS succesfully defragments free space between the volume releasing iterations. On the other hand, while examining evolution of free space fragmentation of EXT4 (Figure~\ref{ext499_free}, last 6 hours of the test), we can see that after volume releasing, the amount of small extents of free space remain the same. Furthermore, through most of the testing time EXT4 contained about 20 thousands of extents of free space, which is significantly more than XFS.
...
\begin{figure}
    ...
    \caption{Evolution of latency of create operation, in untrimed file system (XFS)}
...
The file system aging test use workload generator to simulate aging. Because test is automated, environment configuration has to be done as well as tool preparation. While aging, the test gathers lots of data and statistics about file system usage, fragmentation levels and performance of conducted IO operations. After the test, metadata images of file systems are atomatically created and compresed. All the generated data is then compressed and sent to remote server together with file system image.

This file system aging test succesfully induce fragmentation and aging of used file systems. From the generated data, it can be observed, that aging indeed causes performance penalty on file systems. These effects can be observed in both tested file systems.
...
This thesis is aimed on testing so called local file systems with simple configuration, which means, whole device is presented as %one partition% and file system operates on the whole available volume. However, storage usage is evolving into new approaches. 

Cloud solutions are becoming more and more popular, therefore file systems, which operate over storage clusters are already in production (e.g. Ceph). Such file systems have to deal with many new problems, like working with metadata, which are stored on remote server, or distribute data evenly accross the cluster.
...
With such growing complexity, file system aging (and software aging generally) can have signifficant impact on performance of these systems. It is, therfore, very important to research and understand file system aging XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX.
...
\begin{figure}[!htb]
        ...
        \caption[Free space fragmentation of XFS during medium uitlisation test]{On this figure we can see tendency of XFS to merge free space blocks into very large extents (up to 63bg). Furthermore, low fragmentation can be observed during tesing time, with quite large blocks even at the end of the test. This chart displays last 7 hours of the test.}
...
\begin{figure}[!htb]
    ...
    \caption[Size distribution of file extents of XFS during medium utilisation test]{On this chart, we can see that despite low fragmentation of free space, about 333 thousands of extents were present in the final block layout. However, 92\% of files were optimally allocated.}
...
\begin{figure}[!htb]
    \begin{minipage}{\textwidth}
        ...
        \caption[Usage of available volume of XFS during medium utilisation test]{File system utilisation over time. We can see slight curvature of the line, meaning, growth of file system is slowing down. This can be caused by progressively more succesfull delete operation (with more files, there is greatear probability of deleting) and by progressively slower block allocating operations (create, append, random write)}
...
\begin{figure}[h]
    ...
    \caption[Evolution of free space fragmentation of XFS during high utilisation test]{On this chart, we can see evolution of free space fragmentation in the last 5 hours of the test. When comparing this chart with Figure~\ref{fig:usage99_xfs}, we can see that peak in number of extents always appear when the test is randomly deleting space. After that, XFS tries to merge small free space extents into larger ones, succesfully defragmenting free space.}
...
\begin{figure}[h]
    ...
    \caption{Size distribution of file extents}
...
\begin{figure}[!h]
    \begin{minipage}{\textwidth}
        ...
        \caption{Evolution of usage during test}
...
\section{EXT4, HDD, high utilisation}
\begin{itemize}
\itemsep0em 
   \item System: Machine\,3
   \item OS: RHEL-7.3
...
\begin{figure}[h]
    ...
    \caption{Evolution of free space fragmentation}
...
\begin{figure}[h]
    \centering
    \fbox{\includegraphics[width=\textwidth,keepaspectratio]{../charts/SSD_xfs_notrim/free}}
    \caption{Evolution of free space fragmentation}
