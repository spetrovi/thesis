...
The file sysem aging tool succesfully induced fragmentation in all performed tests. From the data generated by aging tool, it is apparent, that the aging process negatively affects performance of the file systems. Performance degradation varies with used storage, file system implementation and maintainance techniques (i.e. \texttt{fstrim}).

The greatest performance penalty can be seen while testing on HDD using sequential operations. Figure X shows evolution of sequential read on SAS HDD. By examining linear regression, we can conclude, that latency raised by 50\% during the test.
