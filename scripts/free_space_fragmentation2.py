...
ffh = read_file('free_frag_histogram.log','r')

ffh = ffh.split('\n')[:-1]
ffh = filter(lambda x: x.find('from') < 0 and x.find('free') < 0, ffh)
...
for i in range(0,len(histogram)):
  ...
  if len(tick[0]) < 3: ticks.append(str(tick[0])+'K-'+str(tick[1])+'K')
  if len(tick[0]) >= 3 and len(tick[0]) < 6: ticks.append(str(int(tick[0])/1000)+'M-'+str(int(tick[1])/1000)+'M')
  if len(tick[0]) >= 6: ticks.append(str(int(tick[0])/1000000)+'G-'+str(int(tick[1])/1000000)+'G')
  extents.append(histogram[i][1][0])
  blocks.append(histogram[i][1][1])
print ticks
print extents
print blocks
dev = '/dev/sda'
