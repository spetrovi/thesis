...
// Set up the chart
var charts;
$(document).ready(function () {charts= new Highcharts.Chart({
...
$(charts.container).on('mousedown.hc touchstart.hc', function(eStart) {
  eStart = charts.pointer.normalize(eStart);

  var posX = eStart.pageX,
    posY = eStart.pageY,
    alpha = charts.options.chart.options3d.alpha,
    beta = charts.options.chart.options3d.beta,
    newAlpha,
    newBeta,
  ...
  $(document).on({
    'mousemove.hc touchdrag.hc': function(e) {
      ...
      charts.options.chart.options3d.beta = newBeta;

      // Run alpha
      newAlpha = alpha + (e.pageY - posY) / sensitivity;
      charts.options.chart.options3d.alpha = newAlpha;
