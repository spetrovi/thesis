...
image_493_chart= new Highcharts.Chart({
...
$(image_493_chart.container).on('mousedown.hc touchstart.hc', function(eStart) {
  eStart = image_493_chart.pointer.normalize(eStart);

  var posX = eStart.pageX,
    posY = eStart.pageY,
    alpha = image_493_chart.options.chart.options3d.alpha,
    beta = image_493_chart.options.chart.options3d.beta,
    newAlpha,
    newBeta,
  ...
  $(document).on({
    'mousemove.hc touchdrag.hc': function(e) {
      ...
      image_493_chart.options.chart.options3d.beta = newBeta;

      // Run alpha
      newAlpha = alpha + (e.pageY - posY) / sensitivity;
      image_493_chart.options.chart.options3d.alpha = newAlpha;

      image_493_chart.redraw(false);
    },
...
var image_493_button = $('#container').highcharts(),
        type = 1,
        types = ['linear', 'logarithmic'],
        lineColor = 'red';

    $('#image_493_button').click(function () {image_493.yAxis[0].update({
            type: types[type]
        });
type += 1;
