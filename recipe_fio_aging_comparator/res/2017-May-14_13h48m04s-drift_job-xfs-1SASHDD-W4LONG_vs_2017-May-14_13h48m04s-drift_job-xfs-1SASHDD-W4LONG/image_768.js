...
image_768_chart= new Highcharts.Chart({
...
$(image_768_chart.container).on('mousedown.hc touchstart.hc', function(eStart) {
  eStart = image_768_chart.pointer.normalize(eStart);

  var posX = eStart.pageX,
    posY = eStart.pageY,
    alpha = image_768_chart.options.chart.options3d.alpha,
    beta = image_768_chart.options.chart.options3d.beta,
    newAlpha,
    newBeta,
  ...
  $(document).on({
    'mousemove.hc touchdrag.hc': function(e) {
      ...
      image_768_chart.options.chart.options3d.beta = newBeta;

      // Run alpha
      newAlpha = alpha + (e.pageY - posY) / sensitivity;
      image_768_chart.options.chart.options3d.alpha = newAlpha;

      image_768_chart.redraw(false);
    },
...
var image_768_button = $('#container').highcharts(),
        type = 1,
        types = ['linear', 'logarithmic'],
        lineColor = 'red';

    $('#image_768_button').click(function () {image_768.yAxis[0].update({
            type: types[type]
        });
type += 1;
