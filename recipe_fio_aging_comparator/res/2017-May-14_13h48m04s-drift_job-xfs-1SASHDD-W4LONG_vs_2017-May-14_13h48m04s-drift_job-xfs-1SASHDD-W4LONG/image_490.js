...
image_490_chart= new Highcharts.Chart({
...
$(image_490_chart.container).on('mousedown.hc touchstart.hc', function(eStart) {
  eStart = image_490_chart.pointer.normalize(eStart);

  var posX = eStart.pageX,
    posY = eStart.pageY,
    alpha = image_490_chart.options.chart.options3d.alpha,
    beta = image_490_chart.options.chart.options3d.beta,
    newAlpha,
    newBeta,
  ...
  $(document).on({
    'mousemove.hc touchdrag.hc': function(e) {
      ...
      image_490_chart.options.chart.options3d.beta = newBeta;

      // Run alpha
      newAlpha = alpha + (e.pageY - posY) / sensitivity;
      image_490_chart.options.chart.options3d.alpha = newAlpha;

      image_490_chart.redraw(false);
    },
...
var image_490_button = $('#container').highcharts(),
        type = 1,
        types = ['linear', 'logarithmic'],
        lineColor = 'red';

    $('#image_490_button').click(function () {image_490.yAxis[0].update({
            type: types[type]
        });
type += 1;
