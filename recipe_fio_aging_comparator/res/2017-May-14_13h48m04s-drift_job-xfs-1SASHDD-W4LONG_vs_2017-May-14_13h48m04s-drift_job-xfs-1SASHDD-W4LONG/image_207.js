...
image_207_chart= new Highcharts.Chart({
...
$(image_207_chart.container).on('mousedown.hc touchstart.hc', function(eStart) {
  eStart = image_207_chart.pointer.normalize(eStart);

  var posX = eStart.pageX,
    posY = eStart.pageY,
    alpha = image_207_chart.options.chart.options3d.alpha,
    beta = image_207_chart.options.chart.options3d.beta,
    newAlpha,
    newBeta,
  ...
  $(document).on({
    'mousemove.hc touchdrag.hc': function(e) {
      ...
      image_207_chart.options.chart.options3d.beta = newBeta;

      // Run alpha
      newAlpha = alpha + (e.pageY - posY) / sensitivity;
      image_207_chart.options.chart.options3d.alpha = newAlpha;

      image_207_chart.redraw(false);
    },
...
var image_207_button = $('#container').highcharts(),
        type = 1,
        types = ['linear', 'logarithmic'],
        lineColor = 'red';

    $('#image_207_button').click(function () {image_207.yAxis[0].update({
            type: types[type]
        });
type += 1;
